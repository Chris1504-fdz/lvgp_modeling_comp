function output = neg_log_l_noise(hyperparam, p_quant, p_qual, ...
lvs_qual, n_lvs_qual, dim_z, X_quant, X_qual, Y, min_eig, k, M, n_z, noise_var_tr_tilde)
%{
inputs:
...
% ---- NEW: add heteroscedastic nugget -----------------
if ~isempty(noise_var_tr_tilde)
    Sigma_tilde = diag(noise_var_tr_tilde(:));  % k x k
    R = R + Sigma_tilde;
end
% ------------------------------------------------------
