function model = LVGP_fit_noise(X, Y, noise_var_tr, varargin)
...
% Y is standardized to [Y_min, Y_max] in your code
scale2 = (Y_max - Y_min)^2;
noise_std2 = noise_var_tr / scale2;          % standardize
...
for i_try = 1:n_try
    ...
    if i_try == 1
        if parallel && (size(hyper0, 1) > 1)
            parfor i = 1:n_opt_i
                [hyper_sol(i,:), obj_sol(i,1), flag_sol(i,1),...
                    ...
                    @(x) neg_log_l_noise(x, p_quant, p_qual, lvs_qual, n_lvs_qual,...
                    dim_z, X_quant, X_qual, Y, eps_i, k, M, n_z, noise_std2), hyper0(i,:),...
        ...
        f = @(params) neg_log_l_noise(params, p_quant, p_qual, lvs_qual, n_lvs_qual,...
            dim_z, X_quant, X_qual, Y, eps_i, k, M, n_z, noise_std2);
        if parallel && (size(hyper0, 1) > 1)
            parfor i = 1:n_opt_i
