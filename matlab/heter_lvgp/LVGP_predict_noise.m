function pred = LVGP_predict_noise(X_new, noise_var_tr, model, varargin)
...
sigma2 = model.fit_detail.sigma2; % <-- use the sigma2 from fitting
R = model.fit_detail.R; % <-- use the R from fitting
M = model.fit_detail.M; % <-- use the R from fitting
Y = model.fit_detail.Y; % <-- use the R from fitting
% R = R + Sigma/ sigma2 (all in correlation scale)
scale2 = (Y_max - Y_min)^2; % scale for Sigma
Sigma_tr = diag(noise_var_tr / scale2); % n_tr x n_tr
% Sigma_tr = diag(noise_var_tr); % n_tr x n_tr
R = R + Sigma_tr/sigma2;
...
if MSE_on
    ...
    temp = Linv*R_old_new;
    W = 1- LinvM'*temp;
    MSE = sigma2*(R_new_new-temp'*temp+W'*W/MTRinvM)*scale2;
    pred.MSE = MSE;
end
